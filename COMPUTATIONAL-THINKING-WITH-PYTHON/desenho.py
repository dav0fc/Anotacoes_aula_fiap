...
def  varias_linhas(altura,largura):
    matriz2 = []
    lista = []
...
'''
Implemente um funcao posicao(mapa,x,y1). Ao chamar posicao(mapa,x,y), pegamos, no mapa, a linha de índice x, e depois o elemento de indice y dessa linha
...
def tabuleiro_xadrez(tamanho):
    ...
    for i in range(tamanho):
        ...
        pos += 1
        pos2 += 1
    tamanho_metade = tamanho / 2
    tamanho_metade = int(tamanho_metade)
    pos = 2
    pos2  = 0
    tamanho_metade = tamanho / 2
    tamanho_metade = int(tamanho_metade)
    for i in range(tamanho_metade):
        mapa[pos][pos2] = "x"
        pos += 1
        pos2 += 1
    pos = 0
